...
const CART_ROLLING_DIGIT_OFFSETS = [0, 3, 7, 1, 8, 4] as const;
const COLLAPSED_CONFIGURATION_ITEM_COUNT = 2;
...
function ConfigurationAccordion({ item }: { item: SimpleCartItem }) {
  ...
  const visibleDetailRows = showAll
    ? detailRows
    : detailRows.slice(0, COLLAPSED_CONFIGURATION_ITEM_COUNT);
      ...
      {open ? (
        ...
        >
          <div className="mt-1 overflow-hidden rounded-[20px] border border-[#dce8f1] bg-[#f8fbfd] shadow-[0_12px_30px_-26px_rgba(15,45,75,0.4)]">
            ...
            {detailRows.length > COLLAPSED_CONFIGURATION_ITEM_COUNT ? (
              <button
                type="button"
...
function CheckoutAction({
  ...
  return (
    <Link
      ...
      className="group/checkout relative flex h-12 w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-[#258fce] px-4 text-sm font-bold text-white transition-all duration-200 before:absolute before:inset-y-0 before:-left-1/2 before:w-1/3 before:-skew-x-12 before:bg-gradient-to-r before:from-transparent before:via-white/35 before:to-transparent before:transition-transform before:duration-700 hover:-translate-y-0.5 hover:bg-[#1d7fb8] hover:before:translate-x-[470%] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#258fce] focus-visible:ring-offset-2 active:scale-[0.99] motion-reduce:transform-none motion-reduce:transition-none motion-reduce:before:hidden"
    >
...
function CartRollingMetric({ value }: { value: string }) {
  return (
        ...
        {Array.from(value).map((character, index) => {
          ...
          return (
            ...
            >
              ...
              >
                {CART_ROLLING_DIGIT_OFFSETS.map((offset) => (
...
function OrderSummary({
  ...
  const subtotalLabel = formatCartCurrency(subtotal);
  const itemCountLabel = text.itemCount(itemCount);
  const finalTotalLabel = formatCartCurrency(finalTotal);
  const isQuoteLoading = quoteStatus === "loading" && !quote;
      ...
      <div className={flat ? "p-5" : "p-5 sm:p-6"}>
        ...
        <div className={`${hideHeader ? "" : "mt-5"} space-y-3.5 text-sm`}>
          <div className="flex items-center justify-between gap-5 text-slate-600">
            ...
            <span className="font-semibold tabular-nums text-[#10253f]">
              {isQuoteLoading ? (
                <CartRollingMetric value={subtotalLabel} />
              ) : (
                subtotalLabel
              )}
            </span>
          ...
          <div className="flex items-center justify-between gap-5 text-slate-600">
            ...
            <span className="font-semibold tabular-nums text-[#10253f]">
              {isQuoteLoading ? (
                <CartRollingMetric value={itemCountLabel} />
              ) : (
                itemCountLabel
              )}
            </span>
        ...
        <div className="mb-5 mt-5 flex items-end justify-between gap-5">
          ...
          <strong className="text-[25px] font-bold tabular-nums tracking-[-0.035em] text-[#10253f]">
            {isQuoteLoading ? (
              <CartRollingMetric value={finalTotalLabel} />
            ) : (
              finalTotalLabel
            )}
          </strong>
...
export default function CartPage() {
  ...
  const quoteById = useMemo(
    () =>
      new Map(
        status === "success"
          ? (quote?.items.map((item) => [item.cartItemId, item]) ?? [])
          : [],
      ),
    [quote, status],
  );
