import type { CartItemPart, CartItemPartType, SimpleCartItem } from "@/stores/cartStore";
import { isCustomFrameDesignData } from "@/components/studio/design-data";
...
function estimateLegacyFramePrice(item: SimpleCartItem) {
  const accessoriesTotal = (item.accessories ?? []).reduce(
    (sum, accessory) => sum + Math.max(0, accessory.price) * Math.max(1, accessory.quantity ?? 1),
    0,
  );
...
function getLegacyCustomParts(item: SimpleCartItem): CartItemPart[] {
  ...
  if (isCustomFrameDesignData(designData)) {
    ...
    const accessoriesById = new Map((item.accessories ?? []).map((accessory) => [accessory.id, accessory]));
...
export function getCartItemParts(item: SimpleCartItem): CartItemPart[] {
  const storedParts = item.parts
    ?.map((part) => scaleStoredPart(part, item.quantity))
