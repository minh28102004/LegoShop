...
const selectTriggerVariants = cva(
  "flex h-11 w-full items-center justify-between rounded-input border border-input bg-surface px-3 text-left text-body-md text-text-primary shadow-control transition-base focus-visible:border-primary disabled:cursor-not-allowed disabled:bg-surface-soft disabled:opacity-60 data-[placeholder]:text-text-muted",
  {
    variants: {
      state: {
        default: "",
        ...
        <SelectPrimitive.Root {...rootProps}>
          ...
          <SelectPrimitive.Portal>
            <SelectPrimitive.Content
              ...
              sideOffset={6}
              className={cn(
                "z-z-popover max-h-72 min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-input border border-border bg-popover text-popover-foreground shadow-elevated",
                contentClassName,
              )}
            >
              <SelectPrimitive.Viewport className="p-1">
                {options.map((option) => {
                  ...
                  > = {
                    value: option.value,
                    className: cn(
                      "relative flex min-h-10 cursor-pointer select-none items-center rounded-md px-3 py-2 pr-8 text-body-sm outline-none transition-base data-[disabled]:pointer-events-none data-[highlighted]:bg-surface-hover data-[highlighted]:text-primary-dark data-[disabled]:opacity-50",
                      itemClassName,
                    ),
                  };
