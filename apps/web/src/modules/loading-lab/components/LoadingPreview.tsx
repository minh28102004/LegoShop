...
function useLoadingLabCopy() {
  const { dictionary } = useI18n();
  return {
    ...dictionary.loadingLab.preview,
...
export function LoadingPreview({
  ...
  if (activeId === "cube-spin") {
    return (
      <PreviewShell label={copy.loadingCollection}>
        <BrickCubeLoader />
      </PreviewShell>
    );
  }
  ...
  return (
    <div className="grid w-full max-w-content gap-4 rounded-md border border-border bg-background p-6 shadow-lg">
      ...
      <Button className="w-full">
        {isSimulating ? <BouncingBlocksLoader /> : copy.addToCart}
      </Button>
  ...
  return (
    <div className="grid w-full max-w-wide gap-6 rounded-md border border-border bg-background p-6 shadow-lg">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h3 className="text-display-sm text-text-primary">{copy.checkout}</h3>
          <p className="mt-1 text-body-sm text-text-muted">
      ...
      <div className="loading-checkout-rail" aria-label={copy.processingOrder}>
      ...
      >
        {isSimulating ? copy.buildingOrder : copy.checkout}
      </Button>
    </div>
  );
}
...
function BrickStackLoader() {
  ...
  return (
    <div
      className="brick-stack-loader"
      role="status"
      aria-label={copy.buildingFrame}
    >
...
function BrickCubeLoader() {
  ...
  return (
    <div
      className="brick-cube-loader"
      role="status"
      aria-label={copy.loadingCollection}
    >
      <span />
    </div>
  );
