...
import { getDashboardStats } from "@/modules/admin/services/adminApi";
import type { DashboardStats as DashboardStatsPayload } from "@/modules/admin/types/admin.types";
...
function toRevenuePoints(stats: DashboardStatsPayload): RevenuePoint[] {
...
function toStatusPoints(
  distribution: DashboardStatsPayload["orderStatusDistribution"],
  t: (key: string) => string,
): StatusPoint[] {
  return distribution
    .slice()
...
export default function DashboardStats() {
  ...
  if (error || !stats) {
    return (
      <Card className="space-y-4 border-red-200 bg-red-50 text-red-700">
        <p>{error ?? t("dashboard.unavailable")}</p>
        <button
          type="button"
          onClick={() => setRetryVersion((version) => version + 1)}
          className="min-h-11 rounded-xl border border-red-200 bg-white px-4 font-semibold transition hover:bg-red-100"
        >
          {t("dashboard.retry")}
        </button>
      </Card>
  ...
  const revenueData = toRevenuePoints(stats);
  const orderStatusData = toStatusPoints(stats.orderStatusDistribution, t);
